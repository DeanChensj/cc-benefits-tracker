...
export function Header({
  ...
  return (
    <header className={`border-b backdrop-blur-md sticky top-0 z-10 px-4 py-2 sm:py-4 transition-colors duration-300 ${
      ...
      <div className="max-w-4xl mx-auto flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-4">
        <div className="flex items-center gap-3">
          <div className="flex items-center justify-center shrink-0">
            <svg 
              className="w-7 h-7 sm:w-7.5 sm:h-7.5 shrink-0 drop-shadow-[0_2px_8px_rgba(197,160,89,0.15)]" 
              viewBox="0 0 24 24" 
              fill="none" 
          ...
          <div>
            <div className="flex items-baseline gap-2 flex-wrap">
              <h1 className={`text-base sm:text-lg font-bold tracking-tight ${themeClass('text-white', 'text-slate-900')}`}>PerkFolio</h1>
              <span className={`text-[9px] font-black uppercase tracking-widest ${themeClass('text-slate-455', 'text-slate-500')}`}>
                {t('brandSub')}
              </span>
            </div>
            <p className={`text-[10px] sm:text-xs flex items-center gap-1.5 mt-0.5 ${themeClass('text-slate-400', 'text-slate-555')}`}>
        ...
        <div className="flex items-center gap-1 sm:gap-2.5 flex-wrap justify-end sm:justify-start self-end sm:self-auto animate-fade-in">
          {(ownedCards.length > 0 || loyaltyAwards.length > 0) && (
            <button
              onClick={() => setActiveModal('wrapped')}
              className="flex items-center gap-1 px-2 py-1 sm:px-3 sm:py-2 rounded-xl border bg-gradient-to-tr from-purple-600/15 via-indigo-600/10 to-purple-600/15 border-purple-500/30 hover:border-purple-400/50 text-purple-400 hover:text-purple-300 font-extrabold text-[10px] sm:text-xs transition duration-300 active:scale-90 cursor-pointer shadow-md shadow-purple-500/5 animate-pulse"
              title={t('wrapped')}
            >
              <Sparkles className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-amber-400 shrink-0 animate-spin-slow hidden sm:block" />
              <span>{t('wrapped')}</span>
            </button>
          )}
          ...
          {/* Cloud Status Icon */}
          <div 
            className={`p-1 sm:p-1.5 rounded-full text-[10px] font-bold flex items-center justify-center ${
          ...
          >
            {theme === 'dark' ? (
              <Sun className="w-3.5 h-3.5 sm:w-4 sm:h-4 animate-spin-slow" />
            ) : (
          ...
          <div className={`flex items-center rounded-full p-0.5 text-[11px] font-extrabold border ${
            ...
            <button 
              ...
              className={`p-1 sm:p-1.5 rounded-full transition cursor-pointer flex items-center justify-center ${themeClass('hover:bg-slate-800', 'hover:bg-slate-200')}`}
            ...
            <span 
              ...
              className="px-1 sm:px-2 py-1 min-w-[65px] sm:min-w-[75px] text-center font-extrabold text-[9px] sm:text-[9.5px] tracking-wider uppercase cursor-pointer hover:opacity-80 active:scale-95 transition select-none"
